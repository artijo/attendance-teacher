...
<<<<<<< HEAD
  ...
  const filteredStudents = classrooms.classroomMembers.filter(
    (std) =>
      ...
      <div className="p-4 flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-64">
          <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
            <svg
              className="w-4 h-4 text-text-color-alt"
              aria-hidden="true"
    ...
    return (
        <>
            <div className="flex flex-col items-center justify-between gap-4 p-4 md:flex-row">
                ...
                {/* <ExportExcelButton
>>>>>>> 4f9ebcc (add and fix: add filterAttByday page and filterAttBBydayPDF and fix ui attendence pdf)
            ...
            {sliceStudentList.length > 0 ? (
              sliceStudentList.map((std, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">{std.student.fName}</td>
                  <td className="px-6 py-4">{std.student.lName}</td>
                  <td className="px-6 py-4">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        std.behaviourScore >= 80
                          ? "bg-green-100 text-green-800"
                          : std.behaviourScore >= 60
...
=======
            ...
            <div className="overflow-y-auto max-h-[500px]">
                <table className="w-full text-sm text-left">
                    <thead className="sticky top-0 z-10 text-sm text-white uppercase bg-primary">
                        <tr>
                            <th className="px-6 py-3">เลขที่</th>
                            <th className="px-6 py-3">รหัสนักเรียน</th>
                            <th className="px-6 py-3">ชื่อ</th>
                        ...
                        {sliceStudentList.length > 0 ? (
                            sliceStudentList.map((std, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${std.behaviourScore >= 80 ? "bg-green-100 text-green-800" :
                                    ...
                                    <td className="px-6 py-4">
                                        <Link
                                            ...
                                            className="inline-flex items-center gap-2 px-3 py-2 text-xs text-white transition-colors rounded-lg bg-primary hover:bg-accent"
                                        >
                ...
                <div className="flex items-center justify-between p-4 border-t border-line">
                    ...
                    <div className="flex gap-1 text-xs font-medium">
                        <button
                            ...
                            className={`inline-flex size-8 items-center justify-center rounded border border-line bg-white text-text-color rtl:rotate-180 ${currentPage === 1 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-50"
                                }`}
                        ...
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                            <button
                                ...
                                className={`block size-8 rounded ${currentPage === page
                        ...
                        <button
                            onClick={() => setSelectedPage(currentPage + 1)}
                            className={`inline-flex size-8 items-center justify-center rounded border border-line bg-white text-text-color rtl:rotate-180 ${currentPage === totalPages ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-50"
                                }`}
      ...
      {filteredStudents.length > 0 && (
        <div className="flex items-center justify-between p-4 border-t border-line">
          <div className="text-text-color-alt text-sm font-body">
            ...
            {Math.min(currentPage * 20, filteredStudents.length)} จาก{" "}
