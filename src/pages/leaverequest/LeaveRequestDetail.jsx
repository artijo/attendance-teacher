...
function LeaveRequestDetail() {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`${HOSTNAME}/t/leave-requests/${id}`)
            .then((response) => {
                ...
                if (response.data) {
                    ...
                    if (data.studingTime && Array.isArray(data.studingTime) && teacherId) {
                        ...
                        console.log("Filtered studying time entries:", data.studingTime.length);
                    } else {
        ...
        <div>
            <div className="mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-primary font-heading">รายละเอียดคำขอลา</h1>
            ...
            {loading ? (
            ...
            ) : leaveRequest && (!leaveRequest.studingTime || leaveRequest.studingTime.length === 0) ? (
                <div className="bg-white rounded-xl shadow-md p-8 text-center border border-line">
                    ...
                    <h2 className="text-xl font-semibold text-text-color mb-2 font-heading">ไม่พบวิชาที่คุณสอนในคำขอลานี้</h2>
                    <p className="text-text-color-alt font-body mb-6">คำขอลานี้ไม่เกี่ยวข้องกับวิชาที่คุณสอน</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-line">
                            ...
                            <div className="p-6">
                                <h2 className="text-xl font-bold text-text-color font-heading mb-4">ข้อมูลนักเรียน</h2>
                                
                                <div className="flex flex-col gap-4">
                                    <div>
                                        <p className="text-sm text-text-color-alt">รหัสนักเรียน</p>
                                        <p className="text-base font-medium">{leaveRequest.stdId}</p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <p className="text-base">{leaveRequest.student?.email || '-'}</p>
                                    </div>
                                    
                                    <div>
                                        <p className="text-sm text-text-color-alt">เบอร์โทรศัพท์</p>
                                        <p className="text-base">{leaveRequest.student?.tel || '-'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-line mb-6">
                            <div className="h-2 bg-gradient-to-r from-primary to-secondary"></div>
                            <div className="p-6">
                                <h2 className="text-xl font-bold text-text-color font-heading mb-4">รายละเอียดการลา</h2>
                                ...
                                <div className="mb-6">
                                    <p className="text-sm text-text-color-alt mb-1">เหตุผลการลา</p>
                                    <div className="bg-gray-50 p-3 rounded-lg border border-line">
                                        <p className="text-base">{leaveRequest.leaveReason || '-'}</p>
                                    </div>
                                </div>
                                
                                {leaveRequest.LeaveFile && (
                                    <div className="mb-6">
                                        <p className="text-sm text-text-color-alt mb-1">เอกสารแนบ</p>
                                        <div className="flex items-center bg-gray-50 p-3 rounded-lg border border-line">
                                            ...
                                            <a href={`${leaveRequest.LeaveFile}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-accent">
                                                ดูเอกสาร
                                            </a>
                                        </div>
                                    </div>
                                ...
                                <h3 className="text-lg font-bold text-text-color font-heading mb-3 mt-6">รายวิชาที่ขอลา</h3>
                                        ...
                                        <tbody className="bg-white divide-y divide-line">
                                            {leaveRequest.studingTime && leaveRequest.studingTime.length > 0 ? (
                                                leaveRequest.studingTime.map((timeEntry) => (
                                                    <tr key={timeEntry.leaveRequestStudingTimeId} className="hover:bg-gray-50">
                                                        <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-text-color">
                                                            {timeEntry.studingTime?.timetable?.subject?.subCode || '-'}
                                                        </td>
                                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-text-color">
                                                            {timeEntry.studingTime?.timetable?.subject?.subNameThai || '-'}
                                                        </td>
                                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-text-color">
                                                            <div>
                                                                {formatStudyDate(timeEntry.studingTime?.studingTimeDate)}
                                                            </div>
                                                            <div className="text-xs text-gray-500">
                                                                {formatTime(timeEntry.studingTime?.timetable?.timeStart)} - {formatTime(timeEntry.studingTime?.timetable?.timeEnd)}
                                                            </div>
                                                            <div className="text-xs text-gray-500">
                                                                ห้อง {timeEntry.studingTime?.timetable?.classroom?.classLevel}/{timeEntry.studingTime?.timetable?.classroom?.classRoom}
                                                            </div>
                                                            {timeEntry.studingTime?.timetable?.subject?.teacher && (
                                                                <div className="text-xs text-gray-500 italic">
                                                                    คุณครู: {timeEntry.studingTime?.timetable?.subject?.teacher?.fName} {timeEntry.studingTime?.timetable?.subject?.teacher?.lName}
                                                                </div>
                                                            )}
                                                        </td>
                ...
                <div className="bg-white rounded-xl shadow-md p-8 text-center border border-line">
                    ...
                    <h2 className="text-xl font-semibold text-text-color mb-2 font-heading">ไม่พบข้อมูลคำขอลา</h2>
                    <p className="text-text-color-alt font-body mb-6">ไม่พบข้อมูลคำขอลาที่ต้องการ หรือคำขออาจถูกลบไปแล้ว</p>
