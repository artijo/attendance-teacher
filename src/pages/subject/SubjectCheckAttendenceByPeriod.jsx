  ...
  const fetchStudentList = async () => {
    ...
    try {
      const response = await axios.get(
        `${HOSTNAME}/t/attendence/${subject.subId}/${classrooms.classId}`,
      );
      if (response.status === 200) {
        ...
        studentDataSorted.forEach((student) => {
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 font-body">
            <div className="bg-primary/10 px-3 py-1.5 rounded-lg">
              <span className="font-medium text-primary">รหัสวิชา:</span>{" "}
              {subject.subCode}
            </div>
            <div className="bg-secondary/10 px-3 py-1.5 rounded-lg">
              <span className="font-medium text-secondary">ห้อง:</span> ม.
              {classrooms.classLevel}/{classrooms.classRoom}
            </div>
          </div>
        </div>

        <div className="bg-white rounded-lg p-4 mt-6 shadow-sm border border-line">
          <p className="text-lg font-medium text-text-color font-heading">
            {subject.subNameThai}
          </p>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-8 text-center border border-line">
          ...
          <p className="text-text-color-alt font-body">
            ยังไม่มีการบันทึกข้อมูลการเข้าเรียนสำหรับคาบเรียนนี้
          </p>
        </div>
      )}
    </div>
